...
class CalamityPageState extends State<CalamityPage> {
  ...
  Future<void> _fetchRecords() async {
    try {
      final url = Uri.parse('http://localhost/BFEPS-BDRRMC/api/evacuation/get_calamity.php'); 
  ...
  void _deleteCalamity(String id) async {
    ValidationDialog.showDeleteDialog(
      ...
      onConfirm: () async {
        ...
        try {
          final url = Uri.parse('http://localhost/BFEPS-BDRRMC/api/evacuation/delete_calamity.php');
  ...
  List<Map<String, dynamic>> _getPaginatedRecords() {
    ...
    List<Map<String, dynamic>> filteredRecords = _allRecords.where((calamity) {
      ...
      String date = "${calamity['date_time'] ?? ''}".trim().toLowerCase(); 
  ...
  void _goToNextPage() {
    int totalFilteredRecords = _allRecords
        .where((calamity) =>
            calamity['date_time'].toString().toLowerCase().contains(_searchQuery.toLowerCase()) ||
            calamity['calamity_name'].toString().toLowerCase().contains(_searchQuery.toLowerCase()) ||
            calamity['iname'].toString().toLowerCase().contains(_searchQuery.toLowerCase()))
        .length;
...
class CalamityFormState extends State<CalamityForm> {
  ...
  void initState() {
    ...
    if (widget.existingData != null) {
      ...
      dateController.text = widget.existingData!['date_time'] ?? '';
      ...
      selectedCause = widget.existingData!['cause'];
    ...
    if (dateController.text.isNotEmpty &&
      ...
      try {
        ...
        var url = widget.isUpdateMode 
            ? Uri.parse('http://localhost/BFEPS-BDRRMC/api/evacuation/update_calamity.php') 
            : Uri.parse('http://localhost/BFEPS-BDRRMC/api/evacuation/save_calamity.php');
